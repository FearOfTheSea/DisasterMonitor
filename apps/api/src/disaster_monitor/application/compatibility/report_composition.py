...
class CurrentDisasterReportService:
    ...
    def __init__(
        ...
        *,
        ...
        self.dependencies = self._tool_dependencies(self._source_catalog)
        self.workflow = DisasterInvestigationWorkflow(
            build_disaster_tool_registry(self.dependencies)
        )
    ...
    @property
    def event_provider(self) -> DisasterEventProvider:
    ...
    def build_agent_tools(
        ...
        return build_disaster_tool_registry(
            self._tool_dependencies(source_catalog), additional_tools
        )

    def _tool_dependencies(
        self, source_catalog: SourceCatalog
    ) -> DisasterToolDependencies:
        return DisasterToolDependencies(
    ...
    async def execute(self, query: DisasterQuery) -> DisasterReport:
        return await self.workflow.execute(query)
