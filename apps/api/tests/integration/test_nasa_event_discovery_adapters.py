...
async def test_eonet_translates_event_geometry_measurement_and_source_timestamps() -> (
    ...
    assert event.provider_ids == ("eonet:EONET_FIXTURE_1",)
    assert event.lineage_ids == ()
...
@pytest.mark.asyncio
async def test_eonet_admits_valid_gdacs_lineage_and_explicit_closure() -> None:
    payload = _eonet_payload_with_geometry([_eonet_point("2026-08-17T10:00:00Z")])
    event = payload["events"][0]
    assert isinstance(event, dict)
    event["sources"] = [
        {
            "id": "GDACS",
            "url": "https://www.gdacs.org/report.aspx?eventtype=WF&eventid=1031816",
        }
    ]
    event["closed"] = "2026-08-17T11:00:00Z"
    ...
    assert admitted.lineage_ids == ("gdacs:wf:1031816",)
    assert admitted.activity_status is IncidentActivityStatus.ENDED
    await client.aclose()
